...
class CNN(object):
    ...
    # num_classes
    def createCNN(self, output_shape, layer_size = [32, 100],
        loss='categorical_crossentropy'):
        # run a check if this is connected to a TPU
        self.checkEnv()
        
        model = Sequential()
        model.add(Conv2D(layer_size[0], (3, 3), activation='relu', kernel_initializer='he_uniform', input_shape=self.input_shape))
        model.add(BatchNormalization())
        model.add(MaxPooling2D((2, 2)))
        model.add(Flatten())
        model.add(Dense(layer_size[1], activation='relu', kernel_initializer='he_uniform'))
        model.add(BatchNormalization())
